        ...
        case 'library':
            return (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-fade-in">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                <div className="relative flex-grow">
                                    ...
                                    <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-gray-200/50 dark:bg-gray-800/50 border-2 border-gray-300 dark:border-gray-700 rounded-full py-2 pl-10 pr-4 text-gray-800 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-500 w-full" />
                                </div>
                                ...
                                <input type="file" ref={fileInputRef} onChange={handleDirectoryChange} {...({webkitdirectory: '', directory: ''} as any)} multiple className="hidden" />
                            </div>
                        </div>
                         {isScanning && (
                            ...
                            {filteredTracks.length > 0 ? (
                                <ul className="space-y-3">
                                    {filteredTracks.map(track => {
                                        ...
                                        return (
                                            <li key={track.id} onClick={() => setSelectedTrackId(track.id)} className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all duration-200 ease-in-out group ${selectedTrackId === track.id ? 'bg-purple-600/40' : 'bg-gray-200 dark:bg-gray-800/60 hover:bg-gray-300 dark:hover:bg-gray-700/80'}`}>
                    ...
                    {selectedTrack && (<div className="xl:col-span-1 bg-white/50 dark:bg-black/30 backdrop-blur-md border border-purple-300 dark:border-purple-500/20 rounded-2xl p-6 space-y-4 overflow-auto max-h-[80vh] animate-fade-in"><div className="flex justify-between items-start"><div><h3 className="font-orbitron text-xl font-bold text-purple-600 dark:text-purple-300">Metadata Editor</h3><p className="text-sm text-gray-600 dark:text-gray-500 break-all">{selectedTrack.name}</p></div><div className="flex items-center gap-2"><button onClick={() => handleAutoAnalyze(selectedTrack)} disabled={isAnalyzing} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white font-bold py-2 px-3 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-wait" title="Auto-analyze BPM and Key">{isAnalyzing ? <IconLoader className="w-5 h-5 animate-spin" /> : <IconBrain className="w-5 h-5" />}<span className="hidden sm:inline">{isAnalyzing ? 'Analyzing...' : 'Auto-Analyze'}</span></button><button onClick={() => setSelectedTrackId(null)} className="p-2 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white hover:bg-gray-300 dark:hover:bg-gray-700 rounded-full flex-shrink-0"><IconX className="w-5 h-5"/></button></div></div><div className="h-6"><div className="flex items-center gap-2">{saveStatus === 'saving' && <><IconLoader className="w-4 h-4 animate-spin text-yellow-500 dark:text-yellow-400" /><p className="text-sm text-yellow-600 dark:text-yellow-400">Saving...</p></>}{saveStatus === 'saved' && <p className="text-sm text-green-600 dark:text-green-400 animate-fade-in">Saved to database ✅</p>}</div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><MetadataInput label="Genre" icon={<IconTag />} value={selectedTrack.genre || ''} onChange={e => handleUpdateTrack({ ...selectedTrack, genre: e.target.value })} /><MetadataInput label="Mood" icon={<IconTag />} value={selectedTrack.mood || ''} onChange={e => handleUpdateTrack({ ...selectedTrack, mood: e.target.value })} /><MetadataInput label="Key" icon={<IconKey />} value={selectedTrack.key || ''} onChange={e => handleUpdateTrack({ ...selectedTrack, key: e.target.value })} /><MetadataInput label="BPM" icon={<IconClock />} type="number" value={selectedTrack.bpm || 120} onChange={e => handleUpdateTrack({ ...selectedTrack, bpm: parseInt(e.target.value) || 0 })} /></div><textarea value={selectedTrack.notes || ''} onChange={e => handleUpdateTrack({ ...selectedTrack, notes: e.target.value})} placeholder="Notes (e.g., sample info, licensing...)" rows={4} className="w-full bg-gray-200/50 dark:bg-gray-800/50 border-2 border-gray-300 dark:border-gray-700 rounded-lg p-3 text-gray-800 dark:text-gray-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"></textarea><div><h3 className="font-orbitron text-lg font-bold text-purple-600 dark:text-purple-300 mb-3">Project Status</h3><div className="space-y-3"><TaskCheckbox label="Mixed" checked={selectedTrack.status.mixed} onChange={e => handleUpdateTrack({ ...selectedTrack, status: { ...selectedTrack.status, mixed: e.target.checked }})} /><TaskCheckbox label="Mastered" checked={selectedTrack.status.mastered} onChange={e => handleUpdateTrack({ ...selectedTrack, status: { ...selectedTrack.status, mastered: e.target.checked }})} /><TaskCheckbox label="Tagged" checked={selectedTrack.status.tagged} onChange={e => handleUpdateTrack({ ...selectedTrack, status: { ...selectedTrack.status, tagged: e.target.checked }})} /><TaskCheckbox label="Registered" checked={selectedTrack.status.registered} onChange={e => handleUpdateTrack({ ...selectedTrack, status: { ...selectedTrack.status, registered: e.target.checked }})} /></div></div></div>)}
                </div>);
        case 'creative':
            return (
